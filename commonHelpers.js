...
</li>`).join("");i.insertAdjacentHTML("afterbegin",o),document.querySelector(".gallery-link").addEventListener("click",s=>{s.preventDefault()}),b.refresh()}function q(a){l.q=a.replace(/\\s+/g,"+");const o=new URLSearchParams(l);(async()=>(await d.get(`https://pixabay.com/api?${o}`)).data)().then(r=>{c.style.display="none",i.innerHTML="",loadBtn.style.display="block",y.addEventListener(),r.hits.length!==0?S(r.hits):m.error({position:"topRight",message:"Sorry, there are no images matching your search query. Please try again!"})}).catch(r=>console.log("Error:",r))}g.addEventListener("submit",a=>{a.preventDefault(),a.target.elements.search.value!==""&&(c.style.display="inline-block",q(a.target.elements.search.value))});
